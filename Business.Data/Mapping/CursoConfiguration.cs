...
namespace Business.Data.Mapping
{
    public class CursoConfiguration : IEntityTypeConfiguration<Curso>
    {
        public void Configure(EntityTypeBuilder<Curso> builder)
        {
            ...
            builder.HasOne(x => x.Materia).WithMany(x => x.Cursos).HasForeignKey(x => x.MateriaId);
            builder.HasOne(x => x.Comision).WithMany(x => x.Cursos).HasForeignKey(x => x.ComisionId);
